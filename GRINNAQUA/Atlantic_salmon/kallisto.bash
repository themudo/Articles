...
if test -f "$FASTQC1"; then 
	...
	singularity exec /share/apps/singularity/orca-3.simg fastqc -o ${dir} ${read1} 
...
if test -f "$FASTQC2" ; then
	...
	singularity exec /share/apps/singularity/orca-3.simg fastqc -o ${dir} ${read2}
...
FILTERED1=${read1}_filtered.fastq.gz
FILTERED2=${read2}_filtered.fastq.gz
if [[ -f "$FILTERED1" && -f "$FILTERED2" ]]; then
	echo 'Filtered reads already exist; skipping fastp'
else
	singularity exec /share/apps/singularity/fastp_v0.23.4.sif fastp -i ${read1} -I ${read2} -o $FILTERED1 -O $FILTERED2 -q 15 -l 30 -h ${sample}.html
