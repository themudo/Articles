...
#knitr::opts_knit$set(root.dir = "C:/Users/gthemudo/Documents/AQUAVIP/kallisto/ensembl_ref/spleen")
...
knitr::opts_knit$set(root.dir = here::here("../../grinnaqua/kallisto/gills"))
```
...
#dir <- here::here("data")
dir <- here::here("../../grinnaqua/kallisto/gills/")
...
conditions <- conditions[conditions$Tissue == "Gills",]
...
phenotypes <- phenotypes[phenotypes$tissue == "Gills",]
```
...
res_Diet <- results(dds, contrast=c("diet", "MET", "CTRL"))
res_24 <- results(dds, contrast=c("time", "24", "0"))
res_72 <- results(dds, contrast=c("time", "72", "0"))
res_120 <- results(dds, contrast=c("time", "120", "0"))
```
...
```{r}
data$combined <- factor(paste0(data$treatment, data$diet))
...
#This will compute the comparisons between all the groups in the "combined" variable. To extract the results for a specific comparison:
resultsNames(dds)
res_treatment <- results(dds, contrast=c("combined", "InfectedCTRL", "controlCTRL")) 
res_diet <- results(dds, contrast=c("combined", "controlMET", "controlCTRL")) 
res_treatmentxdiet <- results(dds, contrast=c("combined", "InfectedMET", "controlCTRL"))
```
...
res_treatment_annotated <- merge(DataFrame(res_treatment), annotation, all.x = TRUE, by = "row.names")
res_diet_annotated <- merge(DataFrame(res_diet), annotation, all.x = TRUE, by = "row.names")
res_treatmentxdiet_annotated <- merge(DataFrame(res_treatmentxdiet), annotation, all.x = TRUE, by = "row.names")
...
res_treatment_annotated_filtered <- subset(res_treatment_annotated[order(res_treatment_annotated$padj),], padj < 0.05)
res_diet_annotated_filtered <- subset(res_diet_annotated[order(res_diet_annotated$padj),], padj < 0.05)
res_treatmentxdiet_annotated_filtered <- subset(res_treatmentxdiet_annotated[order(res_treatmentxdiet_annotated$padj),], padj < 0.05)

#Finally, we can save the file in a tab-delimited format to allow inspection using Excel or similar

write.table(as.data.frame(res_annotated_filtered),file="grinnaqua_DE_results_gills.txt", sep="\t", row.names = FALSE)
write.table(as.data.frame(res_treatment_annotated_filtered),file="grinnaqua_DE_results_treatment_gills.txt", sep="\t", row.names = FALSE)
write.table(as.data.frame(res_diet_annotated_filtered),file="grinnaqua_DE_results_diet_gills.txt", sep="\t", row.names = FALSE)
write.table(as.data.frame(res_treatmentxdiet_annotated_filtered),file="grinnaqua_DE-results_treatmentxdiet_gills.txt", sep="\t", row.names = FALSE)
```
...
#EnhancedVolcano(res_treatment_annotated, x="log2FoldChange", y="padj", lab = "")
#EnhancedVolcano(res_diet_annotated, x="log2FoldChange", y="padj", lab = "")
#EnhancedVolcano(res_treatmentxdiet_annotated, x="log2FoldChange", y="padj", lab = "")
...
top50_treatment <- res_treatment_annotated_filtered[1:50,1]
#top50_diet <- res_diet_annotated_filtered[1:50,1]
#top50_diet <- res_diet_annotated_filtered[1:9,1]
top50_treatmentxdiet <- res_treatmentxdiet_annotated_filtered[1:50,1]
...
top50_treatment_expression <- assay(vsd[top50_treatment])
#top50_diet_expression <- assay(vsd[top50_diet])
top50_treatmentxdiet_expression <- assay(vsd[top50_treatmentxdiet])
...
top50_treatment_expression  <- top50_treatment_expression - rowMeans(top50_treatment_expression)
#top50_diet_expression  <- top50_diet_expression - rowMeans(top50_diet_expression)
top50_treatmentxdiet_expression  <- top50_treatmentxdiet_expression - rowMeans(top50_treatmentxdiet_expression)

#Now we can draw the heatmap with

pheatmap(top50_expression, cluster_cols=FALSE, fontsize_row=6)
pheatmap(top50_treatment_expression, cluster_cols=FALSE, fontsize_row=6)
#pheatmap(top50_diet_expression, cluster_cols=FALSE, fontsize_row=6)
pheatmap(top50_treatmentxdiet_expression, cluster_cols=FALSE, fontsize_row=6)

#We can add colours to identify the different sample groups, and also replace the Ensembl IDs with gene names


annot <- as.data.frame(colData(vsd)[, c("treatment","diet", "time")])
##THIS IS NOT WORKING FOR SOME REASON
#gene_names <- res_annotated[1:50,"ensembl_gene_id"]
gene_names <- res_annotated[1:50,"external_gene_name"]
gene_names <- res_treatment_annotated[1:50,"external_gene_name"]
#gene_names <- res_diet_annotated[1:50,"external_gene_name"]
#gene_names <- res_diet_annotated[1:9,"external_gene_name"]
gene_names <- res_treatmentxdiet_annotated[1:50,"external_gene_name"]
...
plotCounts(dds, gene=top50[1], intgroup=c("treatment","diet", "time"))
...
# d <- plotCounts(dds, gene=top50[1], intgroup="treatment", returnData = TRUE)
# ggplot(d, aes(x=time, y=count, col=treatment)) + geom_point(position=position_jitter(w=0.1,h=0), size=1.5) + scale_y_log10() + theme_bw()
# d <- plotCounts(dds, gene=top50[2], intgroup="treatment", returnData = TRUE)
# ggplot(d, aes(x=time, y=count, col=treatment)) + geom_point(position=position_jitter(w=0.1,h=0), size=1.5) + scale_y_log10() + theme_bw()
# d <- plotCounts(dds, gene=top50[3], intgroup="treatment", returnData = TRUE)
# ggplot(d, aes(x=time, y=count, col=treatment)) + geom_point(position=position_jitter(w=0.1,h=0), size=1.5) + scale_y_log10() + theme_bw()
...
Including code to produce the same graphs and tables as g::prof
```{r}
library(gprofiler2)

gost_res <- gost(res_annotated_filtered@listData$Row.names[1:50],organism = 'omykiss',ordered_query = TRUE)
gostplot(gost_res)
p <- gostplot(gost_res, capped = FALSE, interactive = FALSE)
p
